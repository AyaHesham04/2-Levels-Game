...
void DrawElevator(char Y[][160], int rmve, int prevre, int flagElevator, int openDoor)
{
	if (flagElevator == 0)
	{
		//inside
		Y[rmve][146] = 218;
		for (int r = rmve + 1; r < rmve + 4; r++)
		{
			Y[r][146] = '|';
			Y[r][150] = '|';
		}
		Y[rmve][150] = 191;
		for (int c = 147; c < 150; c++)
		{
			Y[rmve][c] = 196;
	...
	if (flagElevator == 1)
	{
		//old inside
		Y[prevre][146] = 218;
		for (int r = prevre + 1; r < prevre + 4; r++)
		{
			Y[r][146] = ' ';
			Y[r][150] = ' ';
		}
...
void MoveElevator(int& rmve, int& prevre, int& flagElevator)
{
	prevre = rmve;
	if (rmve < 22 && flagElevator == 1)
	{
		rmve++;
	}
	else
...
void HarakHeroY(char Y[][160], int& rHero, int& cHero, char mv, int& flagJetpack, int& flagLadder1up, int& ctl1up, int& flagLadder1down, int& ctl1down, int& flagLadder2up, int& ctl2up, int& flagLadder2down, int& ctl2down, int& flagElevator, int& ctel, int& flagHoldRope, int& cthr, int& flagHeroMoveRope, int& ctmr, int& surfacecheck1, int& flagjump, int& ctj, int& surfacecheck, int& flagfall)
{
	...
	if (mv == 'a' && Y[rHero][cHero - 1] == ' ' && Y[rHero + 1][cHero - 2] == ' ' && Y[rHero + 2][cHero - 2] == ' ' || mv == 'a' && cHero == 15 || mv == 'a' && cHero == 14 || mv == 'a' && cHero == 13 || mv == 'a' && cHero == 12 || mv == 'a' && cHero == 71 || mv == 'a' && cHero == 70 || mv == 'a' && cHero == 69 || mv == 'a' && cHero == 68 || mv == 'a' && cHero == 79 || mv == 'a' && cHero == 80 || mv == 'a' && cHero == 81 || mv == 'a' && cHero == 82 || mv == 'a' && cHero == 61 || mv == 'a' && cHero == 59 || mv == 'a' && cHero == 57)
	{
		if (cHero > 1)
		{
			cHero--;
	...
	if (mv == 'd' && Y[rHero][cHero + 1] == ' ' && Y[rHero + 1][cHero + 2] == ' ' && Y[rHero + 2][cHero + 2] == ' ' || mv == 'd' && cHero == 9 || mv == 'd' && cHero == 10 || mv == 'd' && cHero == 11 || mv == 'd' && cHero == 12 || mv == 'd' && cHero == 76 || mv == 'd' && cHero == 77 || mv == 'd' && cHero == 78 || mv == 'd' && cHero == 79 || mv == 'd' && cHero == 80 || mv == 'd' && cHero == 65 || mv == 'd' && cHero == 66 || mv == 'd' && cHero == 67 || mv == 'd' && cHero == 68 || mv == 'd' && cHero == 55 || mv == 'd' && cHero == 57 || mv == 'd' && cHero == 59)
	{
		if (cHero < 160)
		{
			cHero++;
	...
	if (Y[rHero + 3][cHero - 1] == ' ' && Y[rHero + 3][cHero + 1] == ' ' && flagjump == 0 && flagJetpack == 0 && flagHeroMoveRope == 0)
	{
		flagfall = 1;
	}
	else
...
void DisplayY(char y[][160], int& smvc, int& smvr)
{
	system("cls");
	int c;
	if (smvc - 5 >= 0 && smvc + 25 < 160 && smvr - 13 >= 0 && smvr + 4 < 23)
	{
		for (int r = smvr - 13; r < smvr + 4; r++)
		{
...
void main()
{
	...
	int cmvr = 107, prevcr = 0;
	int renemy3 = 20, cenemy3 = 71, direnemy = 1;
	...
	int flagElevator = 0, ctel = 0, openDoor = 0, rmve = 11, prevre = 0, f = 0;
	int flagjump = 0, surfacecheck = 0, surfacecheck1 = 0, ctj = 0, flagfall = 0;
	int smvc=0, phc=0, smvr=0,phr=0;
	char ch;
	char y[24][160];
	...
	for (;;)
	{
		for (; !_kbhit();)
		{
			...
			if (flagjump == 1)
			{
				HarakHeroY(y, rHero, cHero, ch, flagJetpack, flagLadder1up, ctl1up, flagLadder1down, ctl1down, flagLadder2up, ctl2up, flagLadder2down, ctl2down, flagElevator, ctel, flagHoldRope, cthr, flagHeroMoveRope, ctmr, surfacecheck1, flagjump, ctj, surfacecheck, flagfall);
			}
			if (flagfall == 1)
			{
				HarakHeroY(y, rHero, cHero, ch, flagJetpack, flagLadder1up, ctl1up, flagLadder1down, ctl1down, flagLadder2up, ctl2up, flagLadder2down, ctl2down, flagElevator, ctel, flagHoldRope, cthr, flagHeroMoveRope, ctmr, surfacecheck1, flagjump, ctj, surfacecheck, flagfall);
			...
			{
				HarakHeroY(y, rHero, cHero, ch, flagJetpack, flagLadder1up, ctl1up, flagLadder1down, ctl1down, flagLadder2up, ctl2up, flagLadder2down, ctl2down, flagElevator, ctel, flagHoldRope, cthr, flagHeroMoveRope, ctmr, surfacecheck1, flagjump, ctj, surfacecheck, flagfall);
				MoveElevator(rmve, prevre, flagElevator);
			}
			DrawElevator(y, rmve, prevre, flagElevator, openDoor);
			f++;
			...
			//lose when touch the big spike 
			if (rHero == 20 && cHero >= 103 && cHero < 135)
			{
				rHero = 20;
				cHero = 2;
			}
			//lose when fall on enemy hole 
			if (rHero == 20 && cHero >= 69 && cHero < 80)
			{
				rHero = 20;
				cHero = 2;
			}
			...
			//cout << "chero:" << cHero << endl << "rhero: " << rHero;
			if (smvc <= 159)
			{
				if (phc < cHero)
				{
		...
		if (ch == 'o' && cHero == 144 && rHero == 12)
		{
			openDoor = 1;
		}

		if (ch == 'c' && cHero == 148 && rHero == 12)
		{
			openDoor = 0;
		}

		if (ch == 'o' && cHero == 148 && rHero == 12)
		{
			openDoor = 1;
		}

		if (ch == 'c' && cHero == 144 && rHero == 12 || ch == 'c' && cHero == 143 && rHero == 12)
		{
			openDoor = 0;
		}
		//////////////////////////////////////////////////////////////////////////////////////
		if (ch == 'o' && cHero == 148 && rHero == 20)
		{
			openDoor = 1;
		}

		if ((ch == 'c' && cHero == 142 && rHero == 20) || (ch == 'c' && cHero == 143 && rHero == 20) || (ch == 'c' && cHero == 144 && rHero == 20))
		{
			openDoor = 0;
		}

		if (ch == 's' && cHero == 148 && rHero == 12)
		{
			flagElevator = 1;
		}
